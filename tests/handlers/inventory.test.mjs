...
test('inventory export combines account zones, DNS, and SSL', async () => {
  ...
  const ctx = { cf: { zones: { list: jest.fn().mockResolvedValue({ result: [{ id: 'z1', name: 'example.com', status: 'active' }] }) }, dns: { records: { list: jest.fn().mockResolvedValue({ result: [{ name: 'www' }] }) } }, get: jest.fn((path) => path.startsWith('/zones?') ? { result: [{ id: 'z1', name: 'example.com', status: 'active' }] } : path.includes('dns_records') ? { result: [{ name: 'www' }] } : { value: 'full' }) }, action: 'export', opts: { 'account-id': 'a1' }, outputJson: true, toJsonOutput, printer: { log: jest.fn() }, fail: jest.fn() };
  ...
  const empty = { ...ctx, outputJson: true, toJsonOutput: jest.fn(), cf: { ...ctx.cf, get: jest.fn((path) => path.startsWith('/zones?') ? {} : path.includes('dns_records') ? {} : { value: 'full' }), zones: { list: jest.fn().mockResolvedValue({}) } } };
  ...
  const scalar = { ...ctx, outputJson: true, toJsonOutput: jest.fn(), cf: { ...ctx.cf, get: jest.fn((path) => path.startsWith('/zones?') ? { result: [{ id: 'z1', name: 'example.com' }] } : {}), zones: { list: jest.fn().mockResolvedValue({ result: [{ id: 'z1', name: 'example.com' }] }) }, dns: { records: { list: jest.fn().mockResolvedValue({}) } } } };
